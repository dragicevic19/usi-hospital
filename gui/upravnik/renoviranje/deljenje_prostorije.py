...
from model.DTO.deljenje_prostorijeDTO import DeljenjeProstorijeDTO
...
class DeljenjeProstorije:
    ...
    def provera_zauzeca(self, lista_opreme):
        ...
        if ProstorijeService.deljenje_prostorije(lista_opreme):
    ...
    def provera_broja_prostorija(self):
        ProstorijeService.obrisi_sobe(self._prostorija)
        sprat = self._prostorija.get_sprat()
        if ProstorijeService.slobodan_broj_prostorije(sprat, self._novi_br_prostorije1):
            if ProstorijeService.slobodan_broj_prostorije(sprat, self._novi_br_prostorije2):
                return True
            else:
