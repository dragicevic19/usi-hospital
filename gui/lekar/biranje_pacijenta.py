...
class BiranjePacijenta(PrikazPacijenata):
    ...
    def __init__(self, root,korisnik_servis, ulogovani_lekar=None):
        self._korisnik_servis = korisnik_servis
        super().__init__(root,self._korisnik_servis, ulogovani_lekar)
    ...
    def otvori_formu_za_zakazivanje_lop(self):
        try:
            ...
            korisnicko_ime_odabranog = odabrani_korisnik[0]
            self._root.destroy()
            poziv_forme_zahtev_za_pregled_lop(korisnicko_ime_odabranog,self._korisnik_servis)
        except IndexError:
            messagebox.showerror("GRESKA", "Niste odabrali korisnika!")


def poziv_forme_biranje_pacijenata_za_operaciju(root,korisnik_servis, ulogovani_lekar=None):
    app = BiranjePacijenta(root,korisnik_servis, ulogovani_lekar)
    root.mainloop()
