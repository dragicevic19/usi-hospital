...
class PregledZauzeca(PrikazZauzeca):
    ...
    def __init__(self, root, korisnicko_ime_lekara,korisnik_servis,kalendar_servis):
        super().__init__(root, korisnicko_ime_lekara,kalendar_servis)
    ...
    def prikazi_detalje_o_pacijentu(self):
        try:
            ...
            print(pacijent_iz_dogadjaja)
            self._pacijent = self._korisnik_servis.pronadji_korisnika_po_korisnickom_imenu(pacijent_iz_dogadjaja)
            self.pokretanje_prikaza_podataka()
        except IndexError:
            messagebox.showerror("GRESKA", "Niste odabrali pacijenta!")

    def pokretanje_prikaza_podataka(self):
        root2 = Tk()
        root2.geometry('330x260')
        PrikazPodataka(root2, self._pacijent, self._root)
        root2.mainloop()
...
class PrikazPodataka(PregledZauzeca):
    ...
    def prikazi_detaljne_podatke(self):
        if self._pacijent:
            ...
            messagebox.showinfo("Obrisan","Pacijent je obrisam iz sistema")

def poziv_forme_detaljni_prikaz_pacijenta(root,lekar,korisnik_servis,kalendar_servis):
    kor_ime = lekar.get_korisnicko_ime()
    PregledZauzeca(root, kor_ime,korisnik_servis,kalendar_servis)
    root.mainloop()
