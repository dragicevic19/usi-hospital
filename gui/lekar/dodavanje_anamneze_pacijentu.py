...
class DodavanjeAnamnezePacijentu(PregledAnamneze):
    ...
    def __init__(self, pacijent, lekar,unos_anamneze_servis):
        self._root2 = Tk()
        ...
        super().__init__(self._root2, pacijent,self._unos_anamneze_servis)
    ...
    def _unesi_anamnezu(self):
        unos_anamneze_dto = UnosAnamnezeDTO(self._lekar, self.polje_za_unos_anamneze.get("1.0", "end-1c")
                                            .replace("\n", " "), self._pacijent)
        self._unos_anamneze_servis.dodaj_anamnezu_svuda(unos_anamneze_dto)
