...
class PocetnaFormaSekretar(ModelPocetne):
    ...
    def postavljanje_dugmica(self):
        b1 = ttk.Button(self._frejm_dugmici, text="Pregledanje notifikacija",
                        command=self.pokretanje_pregledanja_notifikacija)
    ...
    def pokretanje_pregledanja_notifikacija(self):
        ...
        poziv_forme_pregledanje_notifikacija_meni(self._okvir_izvrsavanja,self._injektor.kalendar_servis)
...
def poziv_forme_sekretar(korisnik,injektor):
    root = Tk()
    PocetnaFormaSekretar(root, korisnik,injektor)
    root.mainloop()
